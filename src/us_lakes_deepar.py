...
for a, b in data_split_range:
    ...
    # test_dataset = TensorDataset(torch.FloatTensor(tmp_test[0]), torch.FloatTensor(tmp_test[2]))
    ...
    ql_07.append(np.maximum(0.7 * (label - deepar_output[..., 3]), (1-0.7)*(deepar_output[..., 3] - label)).mean() * 1000)
    ql_05.append(np.maximum(0.5 * (label - deepar_output[..., 2]), (1-0.5)*(deepar_output[..., 2] - label)).mean() * 1000)
    ql_03.append(np.maximum(0.3 * (label - deepar_output[..., 1]), (1-0.3)*(deepar_output[..., 1] - label)).mean() * 1000)
    ...
    qr_07.append((np.mean(label < deepar_output[..., 3]), 0.7 - np.mean(label < deepar_output[..., 3])))
    qr_05.append((np.mean(label < deepar_output[..., 2]), 0.5 - np.mean(label < deepar_output[..., 2])))
    qr_03.append((np.mean(label < deepar_output[..., 1]), 0.3 - np.mean(label < deepar_output[..., 1])))
